...
public class QueueTruck {
     ...
     */  
    public QueueTruck( int hieght, int length) { // truck parameters
     ...
     */
    public boolean enqueue(MovingBox box) {
        ...
        for(column = 0; column < stack[0].length; column++ ){ //moving rows once inbetted for loop is done
            for(int j = 0; j < stack.length; j++){ //inbetted for loop to add the the callums
                ...
                if((j != 0) && (stack[j - 1][column] != null) && 
                   ...
                   (stack[j][column] == null)/*Moving box wieght !> box weight*/){ //What box we should put in(limiting factors)
